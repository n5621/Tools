function align_nrrd_data(volPath, segBasePath, segIndicesBasePath, nrrdBasePath, studies, segs)
...
[vol, slice_data, image_meta_data] = ...
    ...
    for j=1:number_segs
        ...
        segInfo = dicominfo([segPath filesep segdirectories{segindices(segs(j)),1} filesep '000000.dcm']);
        
        % load sample nrrd object (for meta data)
        nrrddatafilename = '1.nrrd'
        nrrdpath = [nrrdBasePath filesep study_string filesep segdirectories{segindices(segs(j)),1} ]
        [nrrddata, nrrdmeta] = nrrdread([nrrdpath filesep nrrddatafilename]);
        ...
        alignedSegPath = [segBasePath filesep study_string filesep 'Aligned' filesep 'MAT-files' ];
        if ~exist(alignedSegPath,'dir')
            mkdir(alignedSegPath)
        end
        SEGfilename = segdirectories{segindices(segs(j))};
